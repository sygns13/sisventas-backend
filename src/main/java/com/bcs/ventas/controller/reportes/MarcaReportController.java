...
@RestController
@RequestMapping("/marca_reportes")
public class MarcaReportController {
    @Autowired
    private MarcaReportService marcaReportService;
    ...
    public ResponseEntity<byte[]> exportPdf(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization) throws Exception {
        ...
        headers.setContentDispositionFormData("MarcaReporte", "MarcaReporte.pdf");

        this.SetClaims(Authorization);

        return ResponseEntity.ok().headers(headers).body(marcaReportService.exportPdf());
    }
    ...
    public ResponseEntity<byte[]> exportXls(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization) throws Exception {
        ...
        var contentDisposition = ContentDisposition.builder("attachment")
                .filename("MarcaReporte" + ".xlsx").build();
        ...
        return ResponseEntity.ok()
                .headers(headers)
                .body(marcaReportService.exportXls());
    }
}
