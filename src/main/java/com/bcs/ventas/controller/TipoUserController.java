...
public class TipoUserController {
    ...
    @GetMapping
    public ResponseEntity<Page<TipoUser>> listar(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization,
        ...
        Page<TipoUser> resultado = tipoUserService.listar(pageable, buscar);

        return new ResponseEntity<>(resultado, HttpStatus.OK);
    }

    @GetMapping("/listar-all")
    public ResponseEntity<List<TipoUser>> listarAll(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization) throws Exception{
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TipoUser> listarPorId(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization,
                                                    @PathVariable("id") Long id) throws Exception{

        this.SetClaims(Authorization);

        TipoUser obj = tipoUserService.listarPorId(id);

        if(obj == null) {
            throw new ModeloNotFoundException("ID NO ENCONTRADO "+ id);
        }

        return new ResponseEntity<TipoUser>(obj, HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<TipoUser> registrar(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization,
                                                  @Valid @RequestBody TipoUser a) throws Exception{

        this.SetClaims(Authorization);

        a.setId(null);
        TipoUser obj = tipoUserService.registrar(a);
    ...
    public ResponseEntity<Integer> modificar(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization,
        ...
        TipoUser objBD = tipoUserService.listarPorId(a.getId());

        if(objBD == null) {
            throw new ModeloNotFoundException("ID NO ENCONTRADO "+ a.getId());
        }

        int obj = tipoUserService.modificar(a);
    ...
    public ResponseEntity<Void> altabaja(@RequestHeader(HttpHeaders.AUTHORIZATION) String Authorization,
        ...
        tipoUserService.altabaja(id, valor);
